...
# PDF diagnostics
pdf(file.path(output_filepath, paste0(
  gsub("-", "", Sys.Date()),
  "_LassoDiagnostics_Alpha", model_alpha, "_Nboot", n_bootstrap, "_PLL", pll_i, "_Seed", seed_i, ".pdf"
)), width = 6, height = 4)
plot(cv_fit_initial)
title(paste0("Min CVM = ", round(min(cv_fit_initial$cvm), 3)), line = 2.5)
docr_regularization_paths(cv_fit_initial, name_conversion_use, ylim = c(-1, 1))
dev.off()
...
bootstrap_file_name1 <- file.path(output_filepath, paste0(
  gsub("-", "", Sys.Date()),
  "_LassoCalcs_Alpha", model_alpha, "_Nboot", n_bootstrap, "_PLL", pll_i, "_Seed", seed_i, ".Rds"
))
bootstrap_file_name2 <- file.path(output_filepath, paste0(
  gsub("-", "", Sys.Date()),
  "_LassoCoefs_Alpha", model_alpha, "_Nboot", n_bootstrap, "_PLL", pll_i, "_Seed", seed_i, ".Rds"
))
